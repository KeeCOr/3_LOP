...
- 징집 UI를 병력 배치 UI와 유사한 카드형 요약/슬라이더 구조로 바꾸고, 기존처럼 카드 전체를 덮는 내부 오버레이를 제거했다.
...
- 생성한 말판 이미지를 각 타일 좌표에 맞춰 잘라 보이도록 적용해 칸별 색과 지형감이 HTML 타일 안에서도 이어지게 했다.
- 말 피규어 표시 크기를 키워 보드 위 캐릭터 존재감을 강화했다.
- 미점령 땅은 하단 구매가 스트립을 표시하지 않도록 정리하고, 하단 표기는 점령지/시작지 통행세처럼 즉시 위험 판단이 필요한 정보에 집중하도록 했다.
- 주사위, 이동, AI 행동, 용병 결과 같은 진행 알림은 보드 중앙을 덮는 팝업 대신 우상단 토스트로 표시해 중앙 요약 패널과 같은 연출이 반복되지 않게 했다.
